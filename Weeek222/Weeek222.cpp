...
void insert(List** b, date o) {
    if (*b == NULL) { // если нет ни одной записи
        *b = new List;
        (*b)->person = o;
        (*b)->next = NULL;
        return;
    ...
    // если вставляемая запись меньше предыдущей
    ...
    // если есть только одна запись и вставляется новая, большая имеющейся
    ...
    // есть более двух записей

    List* temp = new List;
    temp = *b;
    while (temp) {
        // и вставляется в конец списка, т.е вставляемая запись самая большая
        ...
        // и вставляется в середину списка, между существующими записями
